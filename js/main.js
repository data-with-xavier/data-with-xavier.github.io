document.querySelectorAll('.contact-trigger').forEach(el=>{el.addEventListener('click',e=>{e.preventDefault();document.getElementById('contactModal').classList.add('open');});});
const cModal=document.getElementById('contactModal');
if(cModal){cModal.addEventListener('click',e=>{if(e.target===cModal)cModal.classList.remove('open');});document.getElementById('contactClose')?.addEventListener('click',()=>cModal.classList.remove('open'));document.addEventListener('keydown',e=>{if(e.key==='Escape')cModal.classList.remove('open');});}
